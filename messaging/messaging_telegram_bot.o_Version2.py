...
class TelegramNotifier:
    def __init__(self, token, strategy, data_client):
        ...
        @self.dp.message_handler(lambda msg: msg.text in ["❌ Cancell", "❌ Cancelar"], state="*")
        async def cancel_any(msg: types.Message, state: FSMContext):
            try:
                ...
                if current_state:
                    ...
                    await safe_send(self.bot, msg.chat.id, get_text("no_active_process", chat_id=msg.chat.id), reply_markup=menu_main(msg.chat.id))
            except Exception as e:
