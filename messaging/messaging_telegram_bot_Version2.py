    ...
    def __init__(self, token, strategy, data_client):
        ...
        async def start_signal(msg: types.Message):
            try:
                ...
                await safe_send(self.bot, msg.chat.id, "", reply_markup=menu_cancel(msg.chat.id))  # Só envia o teclado de cancelar, sem texto
            except Exception as e:
        ...
        async def select_mode(callback: types.CallbackQuery, state: FSMContext):
            try:
                ...
                kb.add(*buttons)
                # Aqui, o botão voltar agora realmente volta para o menu anterior (escolha de modo), não cancela tudo
                ...
                await safe_send(self.bot, callback.from_user.id, "", reply_markup=menu_cancel(callback.from_user.id))  # Só teclado de cancelar
            except Exception as e:
        ...
        async def select_symbol(callback: types.CallbackQuery, state: FSMContext):
            try:
                ...
                await state.finish()
                await safe_send(self.bot, callback.from_user.id, get_text("finished_hint", chat_id=callback.from_user.id), reply_markup=menu_main(callback.from_user.id))
            except Exception as e:
