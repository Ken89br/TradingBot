...
class TelegramNotifier:
            ...
            try:
                ...
                if not signal_data:
                    ...
                    try:
                        ...
                        entry_price = latest_candle.get('close', signal_data.get('price', '-'))
                    except Exception:
                    ...
                    signal_data["expire_entry_price"] = entry_price  # normalmente igual para digital/forex instantâneo
        ...
        async def refresh(callback: types.CallbackQuery):
            try:
                ...
                if signal_data:
                    ...
                    expire_entry_time = recommended_entry_time + pd.Timedelta(minutes=1)
                    signal_data["expire_entry_time"] = expire_entry_time.strftime("%Y-%m-%d %H:%M:%S")
                    signal_data["expire_entry_price"] = entry_price
    ...
    async def send_trade_signal(self, chat_id, asset, signal_data):
        ...
        try:
            ...
            confidence = max(0, min(confidence, 100))  # Garante 0-100
        except Exception:
            confidence = 0
        ...
        entry = signal_data.get('entry_price', signal_data.get('price', '-'))
        ...
        # Sincronização: hora recomendada + entry price na mesma linha; expire entry idem
        msg = (
